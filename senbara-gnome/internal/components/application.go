...
func init() {
	var appClassInit gobject.ClassInitFunc = func(tc *gobject.TypeClass, u uintptr) {
		...
		applicationClass.OverrideActivate(func(a *gio.Application) {
			...
			if senbaraApp.w.GoPointer() != 0 {
				senbaraApp.w.Present()
				return
			}

			senbaraApp.onActivate(a)
		})

		applicationClass.OverrideOpen(func(a *gio.Application, filesPtr uintptr, nFiles int32, hint string) {
			senbaraApp := (*application)(unsafe.Pointer(a.GetData(dataKeyGoInstance)))
			senbaraApp.onOpen(a, filesPtr, nFiles, hint)
		})
	}
...
func (a *application) authorize(
	...
	authorizer := newAuthorizer(
		...
		func() string {
			return a.nv.GetVisiblePage().GetTag()
		},
		func(tags []string, position int) {
			a.nv.ReplaceWithTags(tags, int32(position))
		},
		func(ud userData) {
			a.u = ud
		},
		func() string {
			return a.settings.GetString(resources.SettingServerURLKey)
		},
	)
...
func (a *application) onPanic(err error) {
	a.rawError = err.Error()
	i18nErr := L(a.rawError)
	...
	a.mto.AddToast(toast)
}

func (a *application) onActivate(app *gio.Application) {
	ctx := a.ctx
	log := a.log
	level := a.level
	settings := a.settings
	...
	authorize := func(
		...
		return a.authorize(ctx, loginIfSignedOut)
	}
	...
	menusBuilder := gtk.NewBuilderFromResource(resources.ResourceMenusUIPath)
	pageIndexBuilder := gtk.NewBuilderFromResource(resources.ResourcePageIndexUIPath)
	pageWelcomeBuilder := gtk.NewBuilderFromResource(resources.ResourcePageWelcomeUIPath)
	pageConfigServerURLBuilder := gtk.NewBuilderFromResource(resources.ResourcePageConfigServerURLUIPath)
	pagePreviewBuilder := gtk.NewBuilderFromResource(resources.ResourcePagePreviewUIPath)
	pageRegisterBuilder := gtk.NewBuilderFromResource(resources.ResourcePageRegisterUIPath)
	pageConfigInitialAccessTokenBuilder := gtk.NewBuilderFromResource(resources.ResourcePageConfigInitialAccessTokenPath)
	pageExchangeLoginBuilder := gtk.NewBuilderFromResource(resources.ResourcePageExchangeLoginUIPath)
	pageHomeBuilder := gtk.NewBuilderFromResource(resources.ResourcePageHomeUIPath)
	pageExchangeLogoutBuilder := gtk.NewBuilderFromResource(resources.ResourcePageExchangeLogoutUIPath)
	warningButtonsBuilder := gtk.NewBuilderFromResource(resources.ResourceWarningButtonsUIPath)
	...
	b.GetObject("main_window").Cast(&a.w)

	b.GetObject("main_navigation").Cast(&a.nv)

	b.GetObject("main_toasts_overlay").Cast(&a.mto)
	...
	var (
		pageIndex                  adw.NavigationPage
		pageIndexMenuButton        gtk.MenuButton
		pageWelcome                adw.NavigationPage
		pageWelcomeMenuButton      gtk.MenuButton
		pageConfigServerURL        adw.NavigationPage
		pageConfigServerURLMenu    gtk.MenuButton
		pagePreview                adw.NavigationPage
		pagePreviewMenuButton      gtk.MenuButton
		pageRegister               adw.NavigationPage
		pageRegisterMenuButton     gtk.MenuButton
		pageConfigInitialToken     adw.NavigationPage
		pageConfigInitialTokenMenu gtk.MenuButton
		pageExchangeLogin          adw.NavigationPage
		pageExchangeLoginMenu      gtk.MenuButton
		pageHome                   adw.NavigationPage
		homeSidebarPage            adw.NavigationPage
		pageExchangeLogout         adw.NavigationPage
		pageExchangeLogoutMenu     gtk.MenuButton

		serverSetupMenu gio.Menu
		previewMenu     gio.Menu
		authSetupMenu   gio.Menu
	)

	menusBuilder.GetObject("server_setup_menu").Cast(&serverSetupMenu)
	menusBuilder.GetObject("preview_menu").Cast(&previewMenu)
	menusBuilder.GetObject("auth_setup_menu").Cast(&authSetupMenu)

	pageIndexBuilder.GetObject("page_index").Cast(&pageIndex)
	pageIndexBuilder.GetObject("page_index_menu_button").Cast(&pageIndexMenuButton)
	pageIndexMenuButton.SetMenuModel(&serverSetupMenu.MenuModel)

	pageWelcomeBuilder.GetObject("page_welcome").Cast(&pageWelcome)
	pageWelcomeBuilder.GetObject("page_welcome_menu_button").Cast(&pageWelcomeMenuButton)
	pageWelcomeMenuButton.SetMenuModel(&serverSetupMenu.MenuModel)
	pageWelcomeBuilder.GetObject("welcome_get_started_button").Cast(&welcomeGetStartedButton)
	pageWelcomeBuilder.GetObject("welcome_get_started_spinner").Cast(&welcomeGetStartedSpinner)

	pageConfigServerURLBuilder.GetObject("page_config_server_url").Cast(&pageConfigServerURL)
	pageConfigServerURLBuilder.GetObject("page_config_server_url_menu_button").Cast(&pageConfigServerURLMenu)
	pageConfigServerURLMenu.SetMenuModel(&serverSetupMenu.MenuModel)
	pageConfigServerURLBuilder.GetObject("config_server_url_input").Cast(&configServerURLInput)
	pageConfigServerURLBuilder.GetObject("config_server_url_continue_button").Cast(&configServerURLContinueButton)
	pageConfigServerURLBuilder.GetObject("config_server_url_continue_spinner").Cast(&configServerURLContinueSpinner)

	pagePreviewBuilder.GetObject("page_preview").Cast(&pagePreview)
	pagePreviewBuilder.GetObject("page_preview_menu_button").Cast(&pagePreviewMenuButton)
	pagePreviewMenuButton.SetMenuModel(&previewMenu.MenuModel)
	pagePreviewBuilder.GetObject("preview_login_button").Cast(&previewLoginButton)
	pagePreviewBuilder.GetObject("preview_login_spinner").Cast(&previewLoginSpinner)
	pagePreviewBuilder.GetObject("preview_contacts_count_label").Cast(&previewContactsCountLabel)
	pagePreviewBuilder.GetObject("preview_contacts_count_spinner").Cast(&previewContactsCountSpinner)
	pagePreviewBuilder.GetObject("preview_journal_entries_count_label").Cast(&previewJournalEntriesCountLabel)
	pagePreviewBuilder.GetObject("preview_journal_entries_count_spinner").Cast(&previewJournalEntriesCountSpinner)

	pageRegisterBuilder.GetObject("page_register").Cast(&pageRegister)
	pageRegisterBuilder.GetObject("page_register_menu_button").Cast(&pageRegisterMenuButton)
	pageRegisterMenuButton.SetMenuModel(&previewMenu.MenuModel)
	pageRegisterBuilder.GetObject("register_register_button").Cast(&registerRegisterButton)

	pageConfigInitialAccessTokenBuilder.GetObject("page_config_initial_access_token").Cast(&pageConfigInitialToken)
	pageConfigInitialAccessTokenBuilder.GetObject("page_config_initial_access_token_menu_button").Cast(&pageConfigInitialTokenMenu)
	pageConfigInitialTokenMenu.SetMenuModel(&serverSetupMenu.MenuModel)
	pageConfigInitialAccessTokenBuilder.GetObject("config_initial_access_token_input").Cast(&configInitialAccessTokenInput)
	pageConfigInitialAccessTokenBuilder.GetObject("config_initial_access_token_login_button").Cast(&configInitialAccessTokenLoginButton)
	pageConfigInitialAccessTokenBuilder.GetObject("config_initial_access_token_login_spinner").Cast(&configInitialAccessTokenLoginSpinner)

	pageExchangeLoginBuilder.GetObject("page_exchange_login").Cast(&pageExchangeLogin)
	pageExchangeLoginBuilder.GetObject("page_exchange_login_menu_button").Cast(&pageExchangeLoginMenu)
	pageExchangeLoginMenu.SetMenuModel(&authSetupMenu.MenuModel)
	pageExchangeLoginBuilder.GetObject("exchange_login_cancel_button").Cast(&exchangeLoginCancelButton)
	...
	pageHomeBuilder.GetObject("home_content_page").Cast(&homeContentPage)
	pageHomeBuilder.GetObject("home_user_menu_button").Cast(&homeUserMenuButton)
	pageHomeBuilder.GetObject("home_user_menu_avatar").Cast(&homeUserMenuAvatar)
	pageHomeBuilder.GetObject("home_user_menu_spinner").Cast(&homeUserMenuSpinner)
	pageHomeBuilder.GetObject("home_hamburger_menu_button").Cast(&homeHamburgerMenuButton)
	pageHomeBuilder.GetObject("home_hamburger_menu_icon").Cast(&homeHamburgerMenuIcon)
	pageHomeBuilder.GetObject("home_hamburger_menu_spinner").Cast(&homeHamburgerMenuSpinner)
	pageHomeBuilder.GetObject("home_sidebar_contacts_count_label").Cast(&homeSidebarContactsCountLabel)
	pageHomeBuilder.GetObject("home_sidebar_contacts_count_spinner").Cast(&homeSidebarContactsCountSpinner)
	pageHomeBuilder.GetObject("home_sidebar_journal_entries_count_label").Cast(&homeSidebarJournalEntriesCountLabel)
	pageHomeBuilder.GetObject("home_sidebar_journal_entries_count_spinner").Cast(&homeSidebarJournalEntriesCountSpinner)
	pageHomeBuilder.GetObject("contacts_stack").Cast(&contactsStack)
	pageHomeBuilder.GetObject("contacts_list").Cast(&contactsListBox)
	pageHomeBuilder.GetObject("contacts_searchentry").Cast(&contactsSearchEntry)
	pageHomeBuilder.GetObject("contacts_add_button").Cast(&contactsAddButton)
	pageHomeBuilder.GetObject("contacts_search_button").Cast(&contactsSearchButton)
	pageHomeBuilder.GetObject("contacts_empty_add_button").Cast(&contactsEmptyAddButton)

	pageExchangeLogoutBuilder.GetObject("page_exchange_logout").Cast(&pageExchangeLogout)
	pageExchangeLogoutBuilder.GetObject("page_exchange_logout_menu_button").Cast(&pageExchangeLogoutMenu)
	pageExchangeLogoutMenu.SetMenuModel(&authSetupMenu.MenuModel)
	pageExchangeLogoutBuilder.GetObject("exchange_logout_cancel_button").Cast(&exchangeLogoutCancelButton)

	a.nv.Add(&pageIndex)
	a.nv.Add(&pageWelcome)
	a.nv.Add(&pageConfigServerURL)
	a.nv.Add(&pagePreview)
	a.nv.Add(&pageRegister)
	a.nv.Add(&pageConfigInitialToken)
	a.nv.Add(&pageExchangeLogin)
	a.nv.Add(&pageHome)
	a.nv.Add(&pageExchangeLogout)

	var mainBreakpoint adw.Breakpoint
	b.GetObject("main_breakpoint").Cast(&mainBreakpoint)
	var collapsedValue gobject.Value
	collapsedValue.Init(gobject.TypeBooleanVal)
	collapsedValue.SetBoolean(true)
	mainBreakpoint.AddSetter(&homeSplitView.Object, "collapsed", &collapsedValue)
	...
	pageHomeBuilder.GetObject("contacts_error_status_page").Cast(&contactsErrorStatusPage)
	pageHomeBuilder.GetObject("contacts_error_refresh_button").Cast(&contactsErrorRefreshButton)
	pageHomeBuilder.GetObject("contacts_error_copy_details").Cast(&contactsErrorCopyDetailsButton)
	pageHomeBuilder.GetObject("contacts_view_page_title").Cast(&contactsViewPageTitle)
	pageHomeBuilder.GetObject("contacts_view_stack").Cast(&contactsViewStack)
	pageHomeBuilder.GetObject("contacts_view_error_status_page").Cast(&contactsViewErrorStatusPage)
	pageHomeBuilder.GetObject("contacts_view_error_refresh_button").Cast(&contactsViewErrorRefreshButton)
	pageHomeBuilder.GetObject("contacts_view_error_copy_details").Cast(&contactsViewErrorCopyDetailsButton)
	pageHomeBuilder.GetObject("contacts_view_edit_button").Cast(&contactsViewEditButton)
	contactsViewEditButton.SetActionTargetValue(glib.NewVariantInt64(0))
	pageHomeBuilder.GetObject("contacts_view_delete_button").Cast(&contactsViewDeleteButton)
	contactsViewDeleteButton.SetActionTargetValue(glib.NewVariantInt64(0))
	pageHomeBuilder.GetObject("contacts_view_optional_fields").Cast(&contactsViewOptionalFieldsPreferencesGroup)
	pageHomeBuilder.GetObject("contacts_view_birthday").Cast(&contactsViewBirthdayRow)
	pageHomeBuilder.GetObject("contacts_view_address").Cast(&contactsViewAddressRow)
	pageHomeBuilder.GetObject("contacts_view_notes").Cast(&contactsViewNotesRow)
	pageHomeBuilder.GetObject("contacts_view_debts").Cast(&contactsViewDebtsListBox)
	pageHomeBuilder.GetObject("contacts_view_activities").Cast(&contactsViewActivitiesListBox)
	pageHomeBuilder.GetObject("activities_view_page_title").Cast(&activitiesViewPageTitle)
	pageHomeBuilder.GetObject("activities_view_stack").Cast(&activitiesViewStack)
	pageHomeBuilder.GetObject("activities_view_error_status_page").Cast(&activitiesViewErrorStatusPage)
	pageHomeBuilder.GetObject("activities_view_error_refresh_button").Cast(&activitiesViewErrorRefreshButton)
	pageHomeBuilder.GetObject("activities_view_error_copy_details").Cast(&activitiesViewErrorCopyDetailsButton)
	pageHomeBuilder.GetObject("activities_view_edit_button").Cast(&activitiesViewEditButton)
	activitiesViewEditButton.SetActionTargetValue(glib.NewVariantInt64(0))
	pageHomeBuilder.GetObject("activities_view_delete_button").Cast(&activitiesViewDeleteButton)
	activitiesViewDeleteButton.SetActionTargetValue(glib.NewVariantInt64(0))
	pageHomeBuilder.GetObject("activities_view_body").Cast(&activitiesViewPageBodyWebView)
	{
		...
		onDecidePolicy := func(_ webkit.WebView, decisionPtr uintptr, decisionType webkit.PolicyDecisionType) bool {
			if decisionType == webkit.PolicyDecisionTypeNavigationActionValue {
				...
				if openExternally {
					gtk.ShowUri(&a.w.ApplicationWindow.Window, u.String(), uint32(gdk.CURRENT_TIME))
					return true
				}
	...
	pageHomeBuilder.GetObject("activities_edit_page_title").Cast(&activitiesEditPageTitle)
	pageHomeBuilder.GetObject("activities_edit_stack").Cast(&activitiesEditStack)
	pageHomeBuilder.GetObject("activities_edit_error_status_page").Cast(&activitiesEditErrorStatusPage)
	pageHomeBuilder.GetObject("activities_edit_error_refresh_button").Cast(&activitiesEditErrorRefreshButton)
	pageHomeBuilder.GetObject("activities_edit_error_copy_details").Cast(&activitiesEditErrorCopyDetailsButton)
	pageHomeBuilder.GetObject("activities_edit_save_button").Cast(&activitiesEditPageSaveButton)
	pageHomeBuilder.GetObject("activities_edit_save_spinner").Cast(&activitiesEditPageSaveSpinner)
	pageHomeBuilder.GetObject("activities_edit_page_name_input").Cast(&activitiesEditPageNameInput)
	pageHomeBuilder.GetObject("activities_edit_page_date_input").Cast(&activitiesEditPageDateInput)
	pageHomeBuilder.GetObject("activities_edit_page_description_expander").Cast(&activitiesEditPageDescriptionExpander)
	pageHomeBuilder.GetObject("activities_edit_page_description_input").Cast(&activitiesEditPageDescriptionInput)
	warningButtonsBuilder.GetObject("activities_edit_page_date_warning_button").Cast(&activitiesEditPageDateWarningButton)
	warningButtonsBuilder.GetObject("activities_edit_page_date_popover_label").Cast(&activitiesEditPagePopoverLabel)
	pageHomeBuilder.GetObject("debts_edit_page_title").Cast(&debtsEditPageTitle)
	pageHomeBuilder.GetObject("debts_edit_stack").Cast(&debtsEditStack)
	pageHomeBuilder.GetObject("debts_edit_error_status_page").Cast(&debtsEditErrorStatusPage)
	pageHomeBuilder.GetObject("debts_edit_error_refresh_button").Cast(&debtsEditErrorRefreshButton)
	pageHomeBuilder.GetObject("debts_edit_error_copy_details").Cast(&debtsEditErrorCopyDetailsButton)
	pageHomeBuilder.GetObject("debts_edit_save_button").Cast(&debtsEditPageSaveButton)
	pageHomeBuilder.GetObject("debts_edit_save_spinner").Cast(&debtsEditPageSaveSpinner)
	pageHomeBuilder.GetObject("debts_edit_page_you_owe_radio").Cast(&debtsEditPageYouOweRadio)
	pageHomeBuilder.GetObject("debts_edit_page_amount_input").Cast(&debtsEditPageAmountInput)
	pageHomeBuilder.GetObject("debts_edit_page_currency_input").Cast(&debtsEditPageCurrencyInput)
	pageHomeBuilder.GetObject("debts_edit_page_description_expander").Cast(&debtsEditPageDescriptionExpander)
	pageHomeBuilder.GetObject("debts_edit_page_description_input").Cast(&debtsEditPageDescriptionInput)
	pageHomeBuilder.GetObject("debts_edit_page_debt_type_you_owe_row").Cast(&debtsEditPageYouOweActionRow)
	pageHomeBuilder.GetObject("debts_edit_page_debt_type_they_owe_row").Cast(&debtsEditPageTheyOweActionRow)
	pageHomeBuilder.GetObject("contacts_edit_page_title").Cast(&contactsEditPageTitle)
	pageHomeBuilder.GetObject("contacts_edit_stack").Cast(&contactsEditStack)
	pageHomeBuilder.GetObject("contacts_edit_error_status_page").Cast(&contactsEditErrorStatusPage)
	pageHomeBuilder.GetObject("contacts_edit_error_refresh_button").Cast(&contactsEditErrorRefreshButton)
	pageHomeBuilder.GetObject("contacts_edit_error_copy_details").Cast(&contactsEditErrorCopyDetailsButton)
	pageHomeBuilder.GetObject("contacts_edit_save_button").Cast(&contactsEditPageSaveButton)
	pageHomeBuilder.GetObject("contacts_edit_save_spinner").Cast(&contactsEditPageSaveSpinner)
	pageHomeBuilder.GetObject("contacts_edit_page_first_name_input").Cast(&contactsEditPageFirstNameInput)
	pageHomeBuilder.GetObject("contacts_edit_page_last_name_input").Cast(&contactsEditPageLastNameInput)
	pageHomeBuilder.GetObject("contacts_edit_page_nickname_input").Cast(&contactsEditPageNicknameInput)
	pageHomeBuilder.GetObject("contacts_edit_page_email_input").Cast(&contactsEditPageEmailInput)
	pageHomeBuilder.GetObject("contacts_edit_page_pronouns_input").Cast(&contactsEditPagePronounsInput)
	pageHomeBuilder.GetObject("contacts_edit_page_birthday_input").Cast(&contactsEditPageBirthdayInput)
	pageHomeBuilder.GetObject("contacts_edit_page_address_expander").Cast(&contactsEditPageAddressExpander)
	pageHomeBuilder.GetObject("contacts_edit_page_address_input").Cast(&contactsEditPageAddressInput)
	pageHomeBuilder.GetObject("contacts_edit_page_notes_expander").Cast(&contactsEditPageNotesExpander)
	pageHomeBuilder.GetObject("contacts_edit_page_notes_input").Cast(&contactsEditPageNotesInput)
	warningButtonsBuilder.GetObject("contacts_edit_page_email_warning_button").Cast(&contactsEditPageEmailWarningButton)
	warningButtonsBuilder.GetObject("contacts_edit_page_birthday_warning_button").Cast(&contactsEditPageBirthdayWarningButton)
	warningButtonsBuilder.GetObject("contacts_edit_page_birthday_popover_label").Cast(&contactsEditPagePopoverLabel)
	...
	pageHomeBuilder.GetObject("journal_entries_stack").Cast(&journalEntriesStack)
	pageHomeBuilder.GetObject("journal_entries_list").Cast(&journalEntriesListBox)
	pageHomeBuilder.GetObject("journal_entries_searchentry").Cast(&journalEntriesSearchEntry)
	pageHomeBuilder.GetObject("journal_entries_add_button").Cast(&journalEntriesAddButton)
	pageHomeBuilder.GetObject("journal_entries_search_button").Cast(&journalEntriesSearchButton)
	pageHomeBuilder.GetObject("journal_entries_empty_add_button").Cast(&journalEntriesEmptyAddButton)
	pageHomeBuilder.GetObject("journal_entries_error_status_page").Cast(&journalEntriesErrorStatusPage)
	pageHomeBuilder.GetObject("journal_entries_error_refresh_button").Cast(&journalEntriesErrorRefreshButton)
	pageHomeBuilder.GetObject("journal_entries_error_copy_details").Cast(&journalEntriesErrorCopyDetailsButton)
	...
	pageHomeBuilder.GetObject("journal_entries_view_page_title").Cast(&journalEntriesViewPageTitle)
	pageHomeBuilder.GetObject("journal_entries_view_stack").Cast(&journalEntriesViewStack)
	pageHomeBuilder.GetObject("journal_entries_view_error_status_page").Cast(&journalEntriesViewErrorStatusPage)
	pageHomeBuilder.GetObject("journal_entries_view_error_refresh_button").Cast(&journalEntriesViewErrorRefreshButton)
	pageHomeBuilder.GetObject("journal_entries_view_error_copy_details").Cast(&journalEntriesViewErrorCopyDetailsButton)
	pageHomeBuilder.GetObject("journal_entries_view_edit_button").Cast(&journalEntriesViewEditButton)
	journalEntriesViewEditButton.SetActionTargetValue(glib.NewVariantInt64(0))
	pageHomeBuilder.GetObject("journal_entries_view_delete_button").Cast(&journalEntriesViewDeleteButton)
	journalEntriesViewDeleteButton.SetActionTargetValue(glib.NewVariantInt64(0))
	pageHomeBuilder.GetObject("journal_entries_view_body").Cast(&journalEntriesViewPageBodyWebView)
	...
	pageHomeBuilder.GetObject("journal_entries_edit_page_title").Cast(&journalEntriesEditPageTitle)
	pageHomeBuilder.GetObject("journal_entries_edit_stack").Cast(&journalEntriesEditStack)
	pageHomeBuilder.GetObject("journal_entries_edit_error_status_page").Cast(&journalEntriesEditErrorStatusPage)
	pageHomeBuilder.GetObject("journal_entries_edit_error_refresh_button").Cast(&journalEntriesEditErrorRefreshButton)
	pageHomeBuilder.GetObject("journal_entries_edit_error_copy_details").Cast(&journalEntriesEditErrorCopyDetailsButton)
	pageHomeBuilder.GetObject("journal_entries_edit_save_button").Cast(&journalEntriesEditPageSaveButton)
	pageHomeBuilder.GetObject("journal_entries_edit_save_spinner").Cast(&journalEntriesEditPageSaveSpinner)
	pageHomeBuilder.GetObject("journal_entries_edit_page_rating").Cast(&journalEntriesEditPageRatingToggleGroup)
	pageHomeBuilder.GetObject("journal_entries_edit_page_title_input").Cast(&journalEntriesEditPageTitleInput)
	pageHomeBuilder.GetObject("journal_entries_edit_page_body_expander").Cast(&journalEntriesEditPageBodyExpander)
	pageHomeBuilder.GetObject("journal_entries_edit_page_body_input").Cast(&journalEntriesEditPageBodyInput)
	...
	onWelcomeGetStartedClicked := func(_ gtk.Button) {
		a.nv.PushByTag(resources.PageConfigServerURL)
	}
	...
	connectSimpleActionActivate(openPreferencesAction, func() {
		preferencesDialog.Present(&a.w.ApplicationWindow.Window.Widget)
	})
	a.Application.SetAccelsForAction("app.openPreferences", []string{`<Primary>comma`})
	a.Application.AddAction(openPreferencesAction)
	...
	a.Application.AddAction(deregisterClientAction)
	...
	setupAuthn := func(registerClient bool) error {
		...
		if err := a.authner.Init(ctx); err != nil {
			return err
		}
	...
	connectButtonClicked(&configServerURLContinueButton, func() {
		...
		go func() {
			...
			a.nv.PushByTag(resources.PagePreview)
		}()
	})
		...
		go func() {
			...
			if v := spec.Components.SecuritySchemes["oidc"].Value.Extensions[api.OidcDcrInitialAccessTokenPortalUrlExtensionKey]; v != nil {
				...
				if ok {
					...
					a.nv.PushByTag(resources.PageRegister)

					return
				}
			}

			a.nv.PushByTag(resources.PageHome)
		}()
	})
	...
	connectButtonClicked(&registerRegisterButton, func() {
		go func() {
			...
			a.nv.PushByTag(resources.PageConfigInitialAccessToken)
		}()
	})
	...
	connectSimpleActionActivate(selectDifferentServerAction, func() {
		a.nv.ReplaceWithTags([]string{resources.PageWelcome}, 1)
	})
	a.Application.AddAction(selectDifferentServerAction)
	...
	connectButtonClicked(&exchangeLogoutCancelButton, func() {
		a.nv.ReplaceWithTags([]string{resources.PageHome}, 1)
	})
	...
	connectButtonClicked(&contactsAddButton, func() {
		contactsCreateDialog.Present(&a.w.ApplicationWindow.Window.Widget)
	...
	connectButtonClicked(&journalEntriesAddButton, func() {
		journalEntriesCreateDialog.Present(&a.w.ApplicationWindow.Window.Widget)
	...
	createErrAndLoadingHandlers := func(
		...
		connectButtonClicked(errorCopyDetailsButton, func() {
			a.w.GetClipboard().SetText(rawErr)
		})
	...
	connectButtonClicked(&contactsCreateDialogAddButton, func() {
		...
		go func() {
			...
			a.mto.AddToast(adw.NewToast(L("Created Contact")))
	...
	connectButtonClicked(&debtsCreateDialogAddButton, func() {
		...
		go func() {
			...
			a.mto.AddToast(adw.NewToast(L("Created Debt")))
	...
	connectButtonClicked(&activitiesCreateDialogAddButton, func() {
		...
		go func() {
			...
			a.mto.AddToast(adw.NewToast(L("Created Activity")))
	...
	connectButtonClicked(&activitiesEditPageSaveButton, func() {
		...
		go func() {
			...
			a.mto.AddToast(adw.NewToast(L("Updated Activity")))
	...
	connectButtonClicked(&debtsEditPageSaveButton, func() {
		...
		go func() {
			...
			a.mto.AddToast(adw.NewToast(L("Updated Debt")))
	...
	connectButtonClicked(&contactsEditPageSaveButton, func() {
		...
		go func() {
			...
			a.mto.AddToast(adw.NewToast(L("Updated Contact")))
	...
	connectButtonClicked(&journalEntriesCreateDialogAddButton, func() {
		...
		go func() {
			...
			a.mto.AddToast(adw.NewToast(L("Created Journal Entry")))
	...
	connectButtonClicked(&journalEntriesEditPageSaveButton, func() {
		...
		go func() {
			...
			a.mto.AddToast(adw.NewToast(L("Updated Journal Entry")))
	...
	connectSimpleActionActivate(logoutAction, func() {
		a.nv.ReplaceWithTags([]string{resources.PageExchangeLogout}, 1)

		go func() {
			if _, err := gio.AppInfoLaunchDefaultForUri(a.u.LogoutURL, nil); err != nil {
				onPanic(err)
	...
	a.Application.AddAction(logoutAction)
	...
	a.Application.AddAction(licenseAction)
	...
	a.Application.AddAction(privacyAction)
	...
	a.Application.AddAction(tosAction)
	...
	a.Application.AddAction(imprintAction)
	...
	connectSimpleActionActivate(codeAction, func() {
		...
		fileDialogSave(fd, &a.w.Window, func(file *gio.FileBase, err error) {
			go func() {
				...
				a.mto.AddToast(adw.NewToast(L("Downloaded Code")))
			}()
		})
	})
	a.Application.AddAction(codeAction)
	...
	connectSimpleActionActivate(exportUserDataAction, func() {
		...
		fileDialogSave(fd, &a.w.Window, func(file *gio.FileBase, err error) {
			go func() {
				...
				a.mto.AddToast(adw.NewToast(L("Exported User Data")))
			}()
		})
	})
	a.Application.AddAction(exportUserDataAction)
	...
	connectSimpleActionActivate(importUserDataAction, func() {
		...
		fileDialogOpen(fd, &a.w.Window, func(file *gio.FileBase, err error) {
			if err != nil {
				onPanic(err)
			...
			connectAlertDialogResponse(confirm, func(response string) {
				if response == "import" {
					go func() {
						...
						a.mto.AddToast(adw.NewToast(L("Imported User Data")))
			...
			confirm.Present(&a.w.ApplicationWindow.Window.Widget)
		})
	})
	a.Application.AddAction(importUserDataAction)
	...
	connectSimpleActionActivate(deleteUserDataAction, func() {
		...
		connectAlertDialogResponse(confirm, func(response string) {
			if response == "delete" {
				...
				a.mto.AddToast(adw.NewToast(L("Deleted User Data")))
	...
	a.Application.AddAction(deleteUserDataAction)

	aboutAction := gio.NewSimpleAction("about", nil)
	connectSimpleActionActivate(aboutAction, func() {
		aboutDialog.Present(&a.w.ApplicationWindow.Window.Widget)
	})
	a.Application.AddAction(aboutAction)

	quitAction := gio.NewSimpleAction("quit", nil)
	connectSimpleActionActivate(quitAction, func() {
		a.Application.Quit()
	})
	a.Application.SetAccelsForAction("app.quit", []string{`<Primary>q`})
	...
	connectSimpleActionActivate(copyErrorToClipboardAction, func() {
		a.w.GetClipboard().SetText(a.rawError)
	})
	a.Application.AddAction(copyErrorToClipboardAction)
	...
	connectSimpleActionActivateWithParam(deleteContactAction, func(parameter *glib.Variant) {
		...
		connectAlertDialogResponse(confirm, func(response string) {
			if response == "delete" {
				...
				a.mto.AddToast(adw.NewToast(L("Contact Deleted")))
	...
	a.Application.AddAction(deleteContactAction)
	...
	connectSimpleActionActivateWithParam(settleDebtAction, func(parameter *glib.Variant) {
		...
		connectAlertDialogResponse(confirm, func(response string) {
			if response == "delete" {
				...
				a.mto.AddToast(adw.NewToast(L("Settled Debt")))
	...
	a.Application.AddAction(settleDebtAction)
	...
	connectSimpleActionActivateWithParam(deleteActivityAction, func(parameter *glib.Variant) {
		...
		connectAlertDialogResponse(confirm, func(response string) {
			if response == "delete" {
				...
				a.mto.AddToast(adw.NewToast(L("Activity Deleted")))
	...
	a.Application.AddAction(deleteActivityAction)
	...
	connectSimpleActionActivateWithParam(deleteJournalEntryAction, func(parameter *glib.Variant) {
		...
		connectAlertDialogResponse(confirm, func(response string) {
			if response == "delete" {
				...
				a.mto.AddToast(adw.NewToast(L("Journal Entry Deleted")))
	...
	a.Application.AddAction(deleteJournalEntryAction)
	...
	a.Application.AddAction(editActivityAction)
	...
	a.Application.AddAction(editDebtAction)
	...
	a.Application.AddAction(editContactAction)
	...
	a.Application.AddAction(editJournalEntryAction)
	...
	a.Application.AddAction(createItemAction)
	a.Application.SetAccelsForAction("app.createItem", []string{`<Primary>n`})
	...
	a.Application.AddAction(searchListAction)
	a.Application.SetAccelsForAction("app.searchList", []string{`<Primary>f`})
	...
	a.Application.AddAction(editItemAction)
	a.Application.SetAccelsForAction("app.editItem", []string{`<Primary>e`})
	...
	a.Application.AddAction(deleteItemAction)
	a.Application.SetAccelsForAction("app.deleteItem", []string{`<Primary>Delete`})
	...
	a.Application.AddAction(navigateToContactsAction)
	a.Application.SetAccelsForAction("app.navigateToContacts", []string{`<Alt>1`})
	...
	a.Application.AddAction(navigateToJournalAction)
	a.Application.SetAccelsForAction("app.navigateToJournal", []string{`<Alt>2`})
		...
		case resources.PageContactsView:
			go func() {
				...
				onActivated := func(_ adw.ButtonRow) {
					debtsCreateDialog.Present(&a.w.ApplicationWindow.Window.Widget)
				...
				onAddActivityActivated := func(_ adw.ButtonRow) {
					activitiesCreateDialog.Present(&a.w.ApplicationWindow.Window.Widget)
	...
	onNavigation := func() {
		var (
			tag = a.nv.GetVisiblePage().GetTag()
			log = log.With("tag", tag)
		)
		...
		case resources.PageIndex:
			go func() {
				if err := checkSenbaraServerConfiguration(); err != nil {
					...
					a.nv.PushByTag(resources.PageWelcome)
				...
				if strings.TrimSpace(a.u.Email) != "" {
					a.nv.PushByTag(resources.PageHome)
	...
	connectNavigationViewPopped(&a.nv, func(page *adw.NavigationPage) {
		onNavigation()
	...
	connectNavigationViewPushed(&a.nv, onNavigation)
	connectNavigationViewReplaced(&a.nv, onNavigation)

	onNavigation()

	a.Application.AddWindow(&a.w.Window)
}

func (appp *application) onOpen(a *gio.Application, filesPtr uintptr, nFiles int32, hint string) {
	ctx := appp.ctx

	if appp.w.GoPointer() == 0 {
		appp.Application.Activate()
	} else {
		appp.w.Present()
	}
	...
	if err != nil {
		if !errors.Is(err, keyring.ErrNotFound) {
			...
			appp.onPanic(errors.Join(errCouldNotLogin, err))
	...
	nextURL, signedOut, err := appp.authner.Exchange(
		ctx,
	...
	appp.nv.ReplaceWithTags([]string{nextURL}, int32(1))
}
