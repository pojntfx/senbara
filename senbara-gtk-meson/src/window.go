...
var (
	gTypeSenbaraGtkMesonMainApplicationWindow gobject.Type
)

//export senbara_gtk_meson_main_application_window_get_type
func senbara_gtk_meson_main_application_window_get_type() C.ulong {
	return C.ulong(gTypeSenbaraGtkMesonMainApplicationWindow)
}

type senbaraGtkMesonMainApplicationWindow struct {
...
func init() {
	var classInit gobject.ClassInitFunc = func(tc *gobject.TypeClass, u uintptr) {
		...
		gobject.SignalNewv(
			"button-test-clicked",
			gTypeSenbaraGtkMesonMainApplicationWindow,
		...
		objClass.OverrideConstructed(func(o *gobject.Object) {
			...
			w := &senbaraGtkMesonMainApplicationWindow{
			...
			onButtonTestClicked := func(gtk.Button) {
				gobject.SignalEmit(
					o,
					gobject.SignalLookup("button-test-clicked", gTypeSenbaraGtkMesonMainApplicationWindow),
					0,
				)
			}
		...
		objClass.OverrideSetProperty(func(o *gobject.Object, u uint, v *gobject.Value, ps *gobject.ParamSpec) {
			...
			case propertyIdTestButtonSensitive:
				w := (*senbaraGtkMesonMainApplicationWindow)(unsafe.Pointer(o.GetData(dataKeyGoInstance)))
	...
	gTypeSenbaraGtkMesonMainApplicationWindow = gobject.TypeRegisterStaticSimple(
		parentQuery.Type,
		"SenbaraGtkMesonMainApplicationWindow",
		parentQuery.ClassSize,
		&classInit,
		parentQuery.InstanceSize+uint(unsafe.Sizeof(senbaraGtkMesonMainApplicationWindow{}))+uint(unsafe.Sizeof(&senbaraGtkMesonMainApplicationWindow{})),
		&instanceInit,
		0,
	)
}

func (w *senbaraGtkMesonMainApplicationWindow) showToast(message string) {
	w.toastOverlay.AddToast(adw.NewToast(message))
}

//export senbara_gtk_meson_main_application_window_show_toast
func senbara_gtk_meson_main_application_window_show_toast(window unsafe.Pointer, message *C.char) {
	w := (*senbaraGtkMesonMainApplicationWindow)(unsafe.Pointer(gobject.ObjectNewFromInternalPtr(uintptr(window)).GetData(dataKeyGoInstance)))
